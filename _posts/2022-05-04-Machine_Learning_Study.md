...
우리는 **지도 학습**이 두 개의 중요한 하위 분야로 `분류`와 `회귀`로 나뉜다는 것을 배웠다.
...
이 객관적 함수는 종종 우리가 최소화하고자 하는 **손실 또는 비용 함수**이다.
...
2장 섹션에서, 분류를 위한 간단한 기계 학습 알고리즘 훈련에서 **간단한 단층 NN 구조**로 학습을 시작했고 이제 **여러 뉴런을 강력한 NN 아키텍처**에 연결하여 손으로 쓴 숫자 인식과 같은 복잡한 문제를 해결했다.

**딥 러닝에 사용되는 많은 NN 모델의 구성 요소** 중 하나인 인기 있는 `역전파 알고리즘`을 구체화했다. 

이 장에서 역전파 알고리즘에 대해 배운 후, 우리는 더 복잡한 DNN 아키텍처를 탐색할 수 있는 준비가 잘 되었다.

나머지 장에서는 보다 고급 딥 러닝 개념과 다중 계층 NN을 보다 효율적으로 구현하고 훈련할 수 있는 오픈 소스 라이브러리인 PyTorch에 대해 다룰 것이다.
