...
하나의 흥미로운 예는 **단백질-리간드 결합(protein-ligand docking)** 과 **약물 설계**이다.

전산 생물학자들은 약물을 전달하기 위해 수용체에 결합할 분자를 설계한다.
...
유전 알고리즘 개요에는 얼마나 **많은 염색체가 모집단**에 있어야 하는지, **알고리즘을 멈추게 하는 임곗값**은 무엇인지, *재생산*을 위해 어떻게 **염색체를 선택**해야하는지, 염색체를 어떻게 **결합(크로스오버)** 해야하는지, **돌연변이가 발생할 확률**은 어느정도인지, 얼마나 **많은 세대를 테스트**해야하는지를 구현해야한다.
...
class GeneticAlgorithm(Generic[C]) : 
  SelectionType = Enum("SelectionType", "ROULETTE TOURNAMENT HIGHTOURNAMENT")
  ...
  def _pick_high_tournament(self) -> Tuple[C, C] :
  ...
  def _reproduce_and_replace(self) -> None:
      ...
      while len(new_population) < len(self._population):
          ...
          if self._selection_type == GeneticAlgorithm.SelectionType.ROULETTE:
              parents: Tuple[C, C] = self._pick_roulette([x.fitness() for x in self._population])
          elif self._selection_type == GeneticAlgorithm.SelectionType.HIGHTOURNAMENT :
              parents : Tuple[C, C] = self._pick_high_tournament()
          else:
              parents = self._pick_tournament(len(self._population) // 2)
          # 두 부모를 크로스오버
          if random() < self._crossover_chance:
