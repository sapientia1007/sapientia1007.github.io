...
```
```
유향 그래프는 정점 간의 연결선이 방향을 갖고 있는, 즉 에지가 단방향인 경우를 이야기한다.
유향 그래프를 사용할 수 있도록, 에지를 추가할때 한 방향으로만 저장하고, 삭제할때도 해당 방향만 삭제할 수 있게 작성했다.
```
...
```
```
나무위키에서 보았듯이, 임의의 지점에서 출발하야 일곱 개의 다리를 한 번씩만 건너서 원래 위치로 돌아오는 알고리즘을 작성했을때, 
