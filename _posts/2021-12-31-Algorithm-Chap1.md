...
이처럼 **기하급수적으로 커지는 함수 호출 횟수를 줄이기** 위해, 이전에 실행된것과 같은 계산을 수행할 때 다시 계산하지 않고 `저장된 값을 사용`할 수 있는 기술은 **메모이제이션**이다. 
...
이는 *데이터를 압축하고 원래 형식으로 되돌리려면 시간이 걸린는 것을 의미한다.* 즉, **데이터 압축**은 데이터가 빠르게 실행되는 것보다 **작은 저장 공간을 차지**하는 것이 우선순위가 높은 상황에서만 의미가 있다.
