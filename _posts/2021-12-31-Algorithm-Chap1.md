...
* 메모이제이션
  : **파서(언어를 해석하는 프로그램, Parser)의 작업 속도를 높이기** 위해 사용하는데, `최근 계산 결과를 다시 사용할 가능성이 있는 모든 문제`에 유용하다. 즉, 동일한 함수가 한 번 더 호출될때 이전에 함수 호출 결과를 저장했으므로 함수를 다시 실행할 필요가 없다.
...
* 일회용 암호 
  : **원본 데이터를 재구성**하기 위해 `암호화기`와 `복호화기`가 키들 중 하나를 가지고 있어야 하며, 이는 *키를 비밀로 유지*하고자 하는 **암호화 체계의 목표**를 달성하는 데 번거로운 일이다. 
